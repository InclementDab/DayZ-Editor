class EditorHudToolbarController : EditorControllerBase
{
			...
			case "BrushTypeBoxData":
				{
					...
					m_Editor.CommandManager[EditorBrushToggleCommand].SetCanExecute(args.Source.Count() > 0); //(bool(args.Source.Count() > 0))
	...
	void MenuBarExecute(ButtonCommandArgs args)
	{
		...
		if (!EditorHud.CurrentMenu)
		{ //  GetMenu().Type() != GetBoundMenu(args.GetButtonWidget()) removed cause GetBoundMenu is gone
