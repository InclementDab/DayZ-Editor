...
class EditorNetUtils
{
	...
	static void PackTransform(vector position, vector orientation, float scale, out int packedData[4])
	{
		...
		CF_Uint posX = Math.Round(position[0] / POS_X_PRECISION);
		CF_Uint posY = Math.Round((position[1] + POS_Y_OFFSET) / POS_Y_PRECISION);
		CF_Uint posZ = Math.Round(position[2] / POS_Z_PRECISION);
		
		CF_Uint rotYaw = Math.Round((orientation[0] + ROT_OFFSET) / ROT_PRECISION);
		CF_Uint rotPitch = Math.Round((orientation[1] + ROT_OFFSET) / ROT_PRECISION);
		CF_Uint rotRoll = Math.Round((orientation[2] + ROT_OFFSET) / ROT_PRECISION);
		
		CF_Uint scl = Math.Round((scale - SCALE_OFFSET) / SCALE_PRECISION);
