...
vector CalculateCubicBezierPoint(float t, vector p0, vector p1, vector p2, vector p3)
{
    float u = 1 - t;
    float tt = t * t;
    float uu = u * u;
...
vector CalculateQuadraticBezierPoint(float t, vector p0, vector p1, vector p2)
{
    float u = 1 - t;
    float tt = t * t;
    float uu = u * u;
...
void LerpBezier(float duration, vector p0, vector p1, vector p2, vector p3)
{
...
void LerpBezier(float duration, vector p0, vector p1, vector p2)
{
	Debug.DestroyAllShapes();
	Debug.DrawSphere(p0);
	Debug.DrawSphere(p1);
...
class BezierDraw
{
	...
	void RunBezierTimer()
	{
		...
		while (i <= 1.0) {
			...
			old_point = point;
			point = CalculateQuadraticBezierPoint(i, m_LinkRef0.Ptr().GetPosition(), m_LinkRef1.Ptr().GetPosition(), m_LinkRef2.Ptr().GetPosition());
