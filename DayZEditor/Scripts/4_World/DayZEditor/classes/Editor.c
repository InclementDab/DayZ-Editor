...
class Editor : Managed
{
	...
	bool OnMouseEnterObject(Object target, int x, int y, int component_index)
	{
		...
		if (m_EditorHudController.ObjectReadoutName.Contains(".p3d"))
		{ // yeah its hacky but its cool!
	...
	EditorSaveData CreateSaveData(bool selected_only = false)
	{
		...
		foreach (int id, EditorHiddenObjects deleted_object: deleted_objects) {
	...
	/* 	void SelectHiddenObject(EditorHiddenObjects target)
		{
			m_ObjectManager.SelectHiddenObject(target);
		}

		void DeselectHiddenObject(EditorHiddenObjects target)
		{
			m_ObjectManager.DeselectHiddenObject(target);
		}

		void ToggleHiddenObjectSelection(EditorHiddenObjects target)
		{
