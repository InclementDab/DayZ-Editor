class EditorInitFile : EditorFileType
{
		...
		foreach (string line: file_contents_split) {
			...
			if (line.Contains("SpawnObject") && !line.Contains("void") && !line.Contains("static"))
			{
	        	...
	        	*/
				...
				save_data.EditorObjects.Insert(EditorObjectData.Create(tokens[1], tokens[3].ToVector() + GetOffset(tokens[1]), tokens[5].ToVector(), 1, EFE_DEFAULT));
			}
		}
	...
	override void Export(EditorSaveData data, string file, ExportSettings settings)
	{
		...
		foreach (EditorObjectData editor_object: data.EditorObjects) {
			spawn_method.Insert(string.Format("SpawnObject(\"%1\", \"%2\", \"%3\", %4);", editor_object.Type, (editor_object.Position - GetOffset(editor_object.Type)).ToString(false), editor_object.Orientation.ToString(false), editor_object.Scale));
		}
