class EditorObject : EditorWorldObject
{
	...
	void EditorObject(notnull EditorObjectData data)
	{
		...
		// If network light
		if (NetworkLight.Cast(m_WorldObject))
		{
			NetworkLight.Cast(m_WorldObject).Read(m_Data.Parameters);
		}
	...
	{
		if (!ItemBase.Cast(m_WorldObject) || !ItemBase.Cast(m_WorldObject).GetHierarchyParent())
		{ // adding this because of the notnull check in GetEditorObject
			return null;
		}
