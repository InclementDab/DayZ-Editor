modded class DayZIntroScene
{
	...
	void DayZIntroScene()
	{
		...
		m_CharacterPos[1] = GetGame().SurfaceY(m_CharacterPos[0], m_CharacterPos[2]) + 0.75;
		...
		switch (m_CurrentHoliday)
		{
			...
			case EditorHoliday.NEWYEARS:
				{
					m_BigCamera = GetGame().CreateObject("DSLRCameraChristmas", m_CharacterPos, true);
					break;
				}

			case EditorHoliday.ANNIVERSARY:
				{
					m_BigCamera = GetGame().CreateObject("DSLRCameraAnniversary", m_CharacterPos, true);
					break;
				}

			default:
				{
					m_BigCamera = GetGame().CreateObject("BigCamera", m_CharacterPos, true);
					break;
				}
		}

		m_BigCamera.SetPosition(m_CharacterPos);
		m_BigCamera.Update();
	}
	...
	{
		vector randDistance = m_BigCamera.GetWorldPosition();
	...
	void OnUpdate(float timeslice)
	{
		...
		if (!m_ParticleSetup)
		{
			if (m_CurrentHoliday == EditorHoliday.NEWYEARS || m_CurrentHoliday == EditorHoliday.CHRISTMAS)
			{
				...
				Particle.PlayOnObject(ParticleList.EDITOR_SNOW, m_BigCamera, Vector(0, 0, 0));
				Particle.PlayOnObject(ParticleList.EDITOR_SNOWFOG_THIN, m_BigCamera, Vector(0, 0, 0));
			}

			m_ParticleSetup = true;
		}

		vector lookat = vector.Direction(m_BigCamera.GetPosition(), m_Camera.GetPosition() + GetGame().GetPointerDirection() * (CAMERA_DISTANCE_FROM_SCREEN / 2));
		vector pos = m_BigCamera.GetPosition();

		// Makes camera 'hover' in position
		pos[1] = pos[1] + Math.Sin(m_TotalTime * Math.PI) / 1500;
		m_BigCamera.SetPosition(pos);
		m_BigCamera.SetDirection(lookat);
		m_BigCamera.Update();

		// easter egg
		if (KeyState(KeyCode.KC_NUMPADENTER))
		{
			vector ori = m_BigCamera.GetOrientation();
			m_Offset += 10;
			if (m_Offset > 360) m_Offset = 0;
			ori[2] = ori[2] + m_Offset;

			m_BigCamera.SetOrientation(ori);
			m_BigCamera.Update();
		...
		if (m_CameraTimer > 1 && (GetMouseState(MouseState.LEFT) & MB_PRESSED_MASK) && GetGame().GetInput().HasGameFocus())
		{
			...
			switch (m_CurrentHoliday)
			{
				...
				default:
					{
						object_to_drop = GetGame().CreateObjectEx("BigCamera", newcam_pos, ECE_CREATEPHYSICS | ECE_SETUP);
						break;
					}
			}
