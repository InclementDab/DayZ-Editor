	...
	override void OnDragging(out vector transform[4], notnull EditorObject target)
	{
		...
		foreach (EditorObject selected_object: selected_objects) {
			...
			// Handle regular motion for all children
				...
				if (GetEditor().GetEditorHud().GetController().GroundButton) {
					if (GetEditor().GetEditorHud().GetController().MagnetButton) {
				...
				selected_object.PlaceOnSurfaceRotated(selected_transform, selected_transform[3], surface_normal[0] * -1, surface_normal[2] * -1, selected_object.LocalAngle * -1, GetEditor().GetEditorHud().GetController().MagnetButton);
			}	
